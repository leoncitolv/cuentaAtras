...
if (!['cards', 'agenda', 'calendar', 'json'].includes(activeView)) activeView = 'cards';
...
function renderViewSwitch() {
  if (!$(`.view-switch button[data-view="${activeView}"]`)) activeView = 'cards';
...
function renderJsonPreview() {
  ...
  els.jsonPreview.textContent = exportData();
}

function updateEmptyState() {
  const count = filteredReminders().length;
  els.empty.hidden = activeView === 'json' || count !== 0;
}
...
function bindEvents() {
  ...
  $('#quickConfigBtn')?.addEventListener('click', openConfig);
...
function handleIncomingFromGastos() {
  const params = new URLSearchParams(location.search);
  if (!params.has('quick') && !params.has('title') && !params.has('due')) return;
  openForm();
  const title = params.get('title') || 'Nuevo pago';
  const due = params.get('due') || '';
  const category = params.get('category') || 'Tarjetas';
  const notes = params.get('notes') || 'Agregado desde Mis Gastos';
  $('#titleInput').value = title;
  if (due) $('#dueInput').value = due.length === 10 ? `${due}T09:00` : due.slice(0,16);
  $('#categoryInput').value = category;
  $('#notesInput').value = notes;
  selectedColor = category.toLowerCase().includes('tarjeta') ? 'blue' : 'purple';
  buildColorPicker();
  buildNotifyGrid(DEFAULT_NOTIFY);
  history.replaceState({}, document.title, location.pathname);
  toast('Fecha recibida desde Mis Gastos. Revisa y guarda el aviso.');
}
...
bindEvents();
loadReminders().then(handleIncomingFromGastos);
